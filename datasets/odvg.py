...
class ODVGDataset(VisionDataset):
    ...
    Args:
        ...
        segm_masks_dir (string): Directory (relative to root) holding per-image GT
            instance mask .npy files of shape [n_instances, h, w], aligned with the
            instance order in the jsonl. Enabled for mask-branch training.
    ...
    def __init__(
        ...
        root: str,
        anno: str,
        label_map_anno: str | None = None,
        max_labels: int = 80,
        segm_masks_dir: str | None = None,
        ...
        self.segm_masks_dir = segm_masks_dir
    ...
    def __getitem__(self, index: int):
        ...
        if self.segm_masks_dir is not None:
            ...
            assert masks.dim() == 3, (
                f"GT masks must be [n_instances, h, w], got {tuple(masks.shape)} "
                f"for {mask_abs_path}"
            )
            assert masks.shape[0] == boxes.shape[0], (
                f"GT mask count {masks.shape[0]} != instance count {boxes.shape[0]} "
                f"for {mask_abs_path}"
            )
            assert masks.shape[1] == h and masks.shape[2] == w, (
                f"GT mask spatial size {(masks.shape[1], masks.shape[2])} != "
                f"image size {(h, w)} for {mask_abs_path}"
            )
            target["masks"] = masks
...
def build_odvg(image_set, args, datasetinfo):
    ...
    if "segm_masks_dir" in datasetinfo:
        segm_masks_dir = datasetinfo["segm_masks_dir"]
    elif getattr(args, "generate_mask", False) and image_set == "train":
        segm_masks_dir = "segm_masks"
    else:
        segm_masks_dir = None
    try:
